...
router.put('/:id', async (request: Request, response: Response) => {
	try {
		...
		// // Atualizar a quantidade de itens no depósito
		// if (requestBody.itensUsados) {
		// 	for (const itemUsado of requestBody.itensUsados) {
		// 		const depositoItem = await prisma.depotItem.findUnique({
		// 			where: { id: itemUsado.id },
		// 		});
		// 		if (depositoItem) {
		// 			await prisma.depotItem.update({
		// 				where: { id: itemUsado.id },
		// 				data: {
		// 					quantity: depositoItem.quantity - itemUsado.quantidadeUsada,
		// 				},
		// 			});
		// 		}
		// 	}
